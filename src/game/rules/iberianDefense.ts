...
export const formIberianDefense = (state: GameState, allies: IberianAllies): GameState => {
  ...
  let next: GameState = {
    ...
    ...(ugtPool ? { armedForces: { ...state.armedForces, militias: { ...state.armedForces.militias, ugt: ugtPool.manpower - leftSocialistReserve }, entityPools: { ...state.armedForces.entityPools,
      ugt_socialist_militias: { ...ugtPool, manpower: ugtPool.manpower - leftSocialistReserve } } } } : {}),
    organizations: { ...state.organizations, ...(state.organizations.PSOE_MILITIA && ugtPool ? { PSOE_MILITIA: { ...state.organizations.PSOE_MILITIA, militiaManpower: ugtPool.manpower - leftSocialistReserve } } : {}) },
