...
const getImageUrl = (url?: string) => {
  ...
  if (url.startsWith(base)) return `${url}?v=2`;
  if (url.startsWith('/')) return `${url}?v=2`;
  return `${base}${url}?v=2`;
};
                ...
                <>
                  ...
                  {advisor.image && (
                    <div className="w-20 aspect-[3/4] border-2 border-ink mb-1 flex-shrink-0 bg-paper-dark overflow-hidden relative">
                      <img 
                        src={getImageUrl(advisor.image)} onError={() => console.error('Failed to load image:', getImageUrl(advisor.image))}
                        alt={advisor.name} 
                        className={cn(
              ...
              {selectedAdvisor.image && (
                <div className="w-full md:w-1/3 flex-shrink-0 relative">
                  <div className="border-2 border-ink bg-white p-2 shadow-[4px_4px_0px_0px_rgba(26,26,26,1)] transform -rotate-2">
                    <img 
                      src={getImageUrl(selectedAdvisor.image)} onError={() => console.error('Failed to load selected image:', getImageUrl(selectedAdvisor.image))}
